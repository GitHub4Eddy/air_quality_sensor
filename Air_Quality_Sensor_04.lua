...
function QuickApp:getQuickAppVariables()
  ...
  if bme280Sensor == "true" then bme280Sensor = true else bme280Sensor = false end
    ...
    http:request(url, {
        ...
        success = function(response)
          ...
          if bme280Sensor then 
            data.pressure = string.format("%.1f",tonumber(jsonTable.sensordatavalues[4].value)/100)
            data.humidity = jsonTable.sensordatavalues[5].value
